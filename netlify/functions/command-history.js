...
exports.handler = async (event) => {
  ...
  try {
    ...
    const history = await db.collection('command_history').find(filter, { projection: { _id: 0, commandId: 1, botId: 1, command: 1, status: 1, actorId: 1, result: 1, error: 1, createdAt: 1 } }).sort({ createdAt: -1 }).limit(40).toArray();

...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      const records = await access.find({}, { projection: { _id: 0, userId: 1, role: 1, permissions: 1, guildIds: 1, enabled: 1, updatedAt: 1 } }).sort({ userId: 1 }).toArray();
    ...
    return json(200, { ok: true, permission: { userId: record.userId, role: record.role, permissions: record.permissions, guildIds: record.guildIds, enabled: record.enabled } });
  } catch (error) {
